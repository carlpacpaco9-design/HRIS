...
import { saveDPCROutputs, submitDPCR, approveDPCR } from '@/app/actions/dpcr'
import { Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle } from "@/components/ui/dialog"

type DPCREditorProps = {
    dpcr: any
    initialOutputs: any[]
    canEdit: boolean
    canApprove: boolean
...
export function DPCREditor({
    ...
    const [outputs, setOutputs] = useState<Output[]>(
        initialOutputs.map(o => ({
            ...
            allotted_budget: o.allotted_budget !== null ? o.allotted_budget : '',
            division_accountable: o.division_accountable || '',
    ...
    const [finalRemarks, setFinalRemarks] = useState(dpcr.final_remarks || '')
    ...
    const saveDraft = async (silent = false) => {
        ...
        const activeOutputs = outputs.filter(o => !o._deleted).map((o, i) => ({
            ...
            success_indicators: o.success_indicators,
            allotted_budget: o.allotted_budget ? Number(o.allotted_budget) : undefined,
            division_accountable: o.division_accountable,
            actual_accomplishments: o.actual_accomplishments,
            remarks: o.remarks
        }))

        const res = await saveDPCROutputs({
            dpcr_form_id: dpcr.id,
    ...
    const handleSubmit = async () => {
        ...
        const res = await submitDPCR(dpcr.id)
        setIsSubmitting(false)
        if (res.success) {
            toast.success('DPCR submitted for approval')
            setIsSubmitOpen(false)
        } else {
            toast.error(res.error || 'Failed to submit DPCR')
        }
    }
    ...
    const handleApprove = async () => {
        ...
        for (const o of activeOpts) {
            if (!o.rating_quantity || !o.rating_efficiency || !o.rating_timeliness) {
                toast.error('Please assign all 3 ratings (Q, E, T) to all active outputs before approving.')
                return
            }
        }
        ...
        const res = await approveDPCR(dpcr.id, payload, finalRemarks)
        if (res.success) toast.success('DPCR Approved')
    ...
    const RatingButtons = ({ val, onSet }: { val: number | null, onSet: (n: number) => void }) => (
        <div className="flex gap-1">
            ...
            ${!canApprove && val !== n ? 'opacity-50 cursor-default' : ''}
          `}
                >
                    {n}
                </button>
    ...
    return (
        <div className="space-y-12">
                ...
                return (
                    <div key={cat} className="space-y-4">
                        <h2 className="text-xl font-bold text-[#1E3A5F] border-b-2 border-slate-200 pb-2 uppercase tracking-wide">
                            {cat} <span className="text-sm font-normal text-slate-500 normal-case ml-2">({catOutputs.length} outputs)</span>
                        </h2>
                            ...
                            {catOutputs.map((out, i) => (
                                <div key={out.front_id} className="bg-white p-5 rounded-lg shadow-sm border border-slate-200 relative">
                                    ...
                                    <div className="grid gap-4">
                                        ...
                                        <div>
                                            <Label className="text-slate-700 font-semibold">Success Indicators (Target + Measure)</Label>
                                            {canEdit ? (
                                                <Textarea
                                                    value={out.success_indicators}
                                                    onChange={e => handleOutputChange(out.front_id, 'success_indicators', e.target.value)}
                                                    rows={3} className="resize-none"
                                                />
                                            ) : (
                                                <p className="text-slate-900 bg-slate-50 p-2 rounded-md border min-h-20 mt-1 whitespace-pre-wrap">{out.success_indicators}</p>
                                            )}
                                        </div>

                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div>
                                                <Label className="text-slate-700 font-semibold">Allotted Budget</Label>
                                                {canEdit ? (
                                                    <Input
                                                        type="number"
                                                        value={out.allotted_budget}
                                                        onChange={e => handleOutputChange(out.front_id, 'allotted_budget', e.target.value)}
                                                        placeholder="e.g. 50000"
                                                    />
                                                ) : (
                                                    <p className="text-slate-900 bg-slate-50 p-2 rounded-md border min-h-10 mt-1">
                                                        {out.allotted_budget ? `₱${Number(out.allotted_budget).toLocaleString()}` : <span className="text-slate-400 italic">None</span>}
                                                    </p>
                                                )}
                                            </div>
                                            <div>
                                                <Label className="text-slate-700 font-semibold">Division / Unit Accountable</Label>
                                                {canEdit ? (
                                                    <Input
                                                        value={out.division_accountable}
                                                        onChange={e => handleOutputChange(out.front_id, 'division_accountable', e.target.value)}
                                                        placeholder="e.g. Administrative Division"
                                                    />
                                                ) : (
                                                    <p className="text-slate-900 bg-slate-50 p-2 rounded-md border min-h-10 mt-1">
                                                        {out.division_accountable || <span className="text-slate-400 italic">None</span>}
                                                    </p>
                                                )}
                                            </div>
                                        </div>
                                    ...
                                    {(canApprove || dpcr.status === 'approved') && (
                ...
                <div className="flex items-center justify-between border-t border-slate-200 pt-6 mt-8">
                    ...
                    <div className="flex gap-4">
                        ...
                        <Button className="bg-[#1E3A5F] hover:bg-[#1E3A5F]/90" onClick={() => setIsSubmitOpen(true)}>
                            <Send className="mr-2 h-4 w-4" /> Submit DPCR
                        </Button>
                    </div>
                </div>
            )}

            {canApprove && dpcr.status === 'submitted' && (
                <div className="bg-white border-2 border-slate-900 rounded-lg p-6 mt-8 shadow-xl">
                    <h3 className="text-lg font-bold text-slate-900 mb-6 flex items-center gap-2"><Star className="h-5 w-5 text-yellow-500 fill-current" /> Approve DPCR</h3>
                    <p className="text-sm text-slate-600 mb-6 font-medium border-b border-slate-200 pb-4">
                        Enter final ratings for each output above, then approve the office performance for this period.
                    </p>
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <Textarea
                                    ...
                                    placeholder="Provide official feedback on office performance..."
                                    rows={4}
            ...
            {dpcr.status === 'approved' && (
                <div className="bg-green-50 border border-green-200 rounded-lg p-6 mt-8 shadow-sm text-center">
                    <p className="text-green-800 font-bold text-xl mb-2">Final Rating: {dpcr.final_average_rating?.toFixed(3)} — {dpcr.adjectival_rating}</p>
                    {dpcr.final_remarks && <p className="text-green-700 italic max-w-2xl mx-auto">"{dpcr.final_remarks}"</p>}
                </div>
            )}
            ...
            <Dialog open={isSubmitOpen} onOpenChange={setIsSubmitOpen}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>Submit DPCR for Approval?</DialogTitle>
                        <DialogDescription>
                            Once submitted, you cannot edit this DPCR until it is approved. Make sure all your accomplishments are accurately reflected.
                        </DialogDescription>
                    </DialogHeader>
