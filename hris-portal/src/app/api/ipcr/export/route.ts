...
export async function GET(req: NextRequest) {
    try {
        ...
        const ipcrId = searchParams.get('ipcr_id')

        if (!ipcrId) return new NextResponse('Missing ipcr_id parameter', { status: 400 })
        ...
        const { data: ipcr, error } = await supabase
            .from('ipcr_forms')
            .select('*, profiles!ipcr_forms_employee_id_fkey(full_name, division, position), immediate_supervisor:profiles!ipcr_forms_immediate_supervisor_id_fkey(full_name), spms_cycles(start_date, end_date)')
            .eq('id', ipcrId)
            .single()

        if (error || !ipcr) return new NextResponse('IPCR not found', { status: 404 })

        let allowed = false
        if (user.id === ipcr.employee_id) allowed = true
        else if (isHRManager(role)) allowed = true
        else if (isDivisionChief(role) && ipcr.profiles.division === profile?.division) allowed = true
        ...
        const startMonth = new Date(ipcr.spms_cycles.start_date).getMonth()
        const periodText = startMonth < 6 ? "January to June" : "July to December"
        const semesterText = startMonth < 6 ? "1ST" : "2ND"
        const year = new Date(ipcr.spms_cycles.start_date).getFullYear()
        ...
        const createCellTextRuns = (runs: TextRun[], colSpan = 1, rowSpan = 1, align: any = AlignmentType.LEFT) => {
            return new TableCell({
                columnSpan: colSpan,
                rowSpan: rowSpan,
        ...
        const createMainContentRows = (opts: any[], emptyRowsCount: number) => {
            ...
            for (const o of opts) {
                ...
                rows.push(new TableRow({
                    children: [
                        ...
                        createCellMultiParagraphs([
                            createText(`Target: ${o.success_indicator_target || ''}`),
                            createText(`Measure: ${o.success_indicator_measure || ''}`)
                        ], 1, 1),
                        createCell("", 1, 1), // Individual/s Accountable (always empty/blank for now or use profile)
            ...
            while (count < emptyRowsCount) {
                count++
                rows.push(new TableRow({
                    children: cw.map(() => createCell('', 1, 1))
                }))
            }
            return rows
        }
            ...
            rows: [
                new TableRow({
                    children: [
                        ...
                        createCell("SUCCESS INDICATORS (TARGETS + MEASURES)", 1, 2, true, AlignmentType.CENTER),
                        createCell("Individual/s Accountable", 1, 2, true, AlignmentType.CENTER),
                ...
                new TableRow({
                    children: [
                        createCell("CORE FUNCTIONS", 9, 1, true, AlignmentType.LEFT, "EEEEEE")
                    ]
                }),
                ...createMainContentRows(coreFunc, 5),
                new TableRow({
                    children: [
                        createCell("SUPPORT FUNCTIONS", 9, 1, true, AlignmentType.LEFT, "EEEEEE")
                    ]
                }),
            ...
            rows: [
                ...
                new TableRow({
                    children: [
                        ...
                        createCell(ipcr.final_average_rating ? ipcr.final_average_rating.toFixed(3) : "", 1, 1, true, AlignmentType.CENTER),
                    ]
                }),
                new TableRow({
                    children: [
                        createCell("ADJECTIVAL RATING", 2, 1, true),
                        createCell(ipcr.adjectival_rating || "", 1, 1, true, AlignmentType.CENTER),
                    ]
                }),
            ]
        })
                ...
                new TableRow({
                    children: [
                        createCellMultiParagraphs([
                            ...
                            createText(""),
                            createText(""),
                            createText(ipcr.profiles?.full_name || "", true, 20, AlignmentType.CENTER),
                            createText("Employee", false, 20, AlignmentType.CENTER),
                        ]),
                        createCellMultiParagraphs([
                            ...
                            createText("Supervisor/Dept Head", false, 20, AlignmentType.CENTER),
                        ]),
                        createCellMultiParagraphs([
                            createText("Approved by:"),
                            createText(""),
                            createText(""),
                            createText("Atty. Mhelygene D. Tesoro-Ramos, REA", true, 20, AlignmentType.CENTER),
                            createText("Head of Office", false, 20, AlignmentType.CENTER),
                        ]),
                    ]
                })
            ]
        ...
        const createP2Rows = (opts: any[], emptyRowsCount: number) => {
            ...
            for (const o of opts) {
                ...
                rows.push(new TableRow({
                    children: [
                        ...
                        createCell(`Target: ${o.success_indicator_target || ''}\nMeasure: ${o.success_indicator_measure || ''}`, 1, 1),
                        createCell(o.actual_accomplishments || "", 1, 1),
                        createCell(o.rating_quantity ? String(o.rating_quantity) : "", 1, 1, false, AlignmentType.CENTER),
                        createCell(o.rating_timeliness ? String(o.rating_timeliness) : "", 1, 1, false, AlignmentType.CENTER),
                        createCell(o.rating_efficiency ? String(o.rating_efficiency) : "", 1, 1, false, AlignmentType.CENTER),
            ...
            while (count < emptyRowsCount) {
                count++
                rows.push(new TableRow({
                    children: p2cw.map(() => createCell('', 1, 1))
                }))
            }
            return rows
        }
        ...
        const doc = new Document({
            sections: [{
                ...
                children: [
                    // PAGE 1
                    createText("INDIVIDUAL PERFORMANCE COMMITMENT AND REVIEW (IPCR)", true, 24, AlignmentType.CENTER),
                    ...
                    new Paragraph({
                        ...
                        children: [
                            ...
                            new TextRun({ text: ipcr.profiles?.full_name || "", font: "Arial", size: 20, bold: true }),
                    ...
                    createText("Legend: 1. Quality  2. Efficiency  3. Timeliness  4. Average", false, 16, AlignmentType.LEFT),
                    ...
                    createText(`Employee: ${ipcr.profiles?.full_name || "______________________"}`, true, 20, AlignmentType.LEFT),
                ]
            }]
        });

        const buffer = await Packer.toBuffer(doc);
        const filename = `IPCR_${ipcr.profiles?.full_name?.replace(/\s+/g, '_') || 'Employee'}_${ipcrId.substring(0, 8)}.docx`;
